...
impl<P: TwoDimensionalPoint> Mesh<P> {
    ...
    pub(crate) fn walker(&self) -> Walker<P> {
    ...
    pub fn walker_from_vertex(&self, vertex_id: VertexID) -> Walker<P> {
        self.walker().into_vertex_halfedge_walker(vertex_id)
    }

    /// Creates a [walker](crate::mesh::traversal::Walker) at the given half-edge.
    pub fn walker_from_halfedge(&self, halfedge_id: HalfEdgeID) -> Walker<P> {
    ...
    pub fn walker_from_face(&self, face_id: FaceID) -> Walker<P> {
...
///
#[derive(Clone, Debug)]
pub struct Walker<'a, P: TwoDimensionalPoint> {
    connectivity_info: &'a ConnectivityInfo<P>,
    current: Option<HalfEdgeID>,
    current_info: Option<HalfEdge>,
}

impl<'a, P: TwoDimensionalPoint> Walker<'a, P> {
    pub(crate) fn new(connectivity_info: &'a ConnectivityInfo<P>) -> Self {
        Walker {
            current: None,
            current_info: None,
            connectivity_info,
