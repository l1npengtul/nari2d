...
impl<P: TwoDimensionalPoint> Mesh<P> {
    ...
    pub fn positions_buffer(&self) -> Vec<f64> {
        ...
        {
            push_2dpoint(&mut positions, position)
        }
        positions
    }
    ...
    pub fn normals_buffer(&self) -> Vec<f64> {
        ...
        for vertex_id in self.vertex_iter() {
            push_2dpoint(&mut normals, self.vertex_normal(vertex_id));
        }
        normals
    }
    ...
    pub fn non_indexed_positions_buffer(&self) -> Vec<f64> {
        ...
        for face_id in self.face_iter() {
            ...
            push_2dpoint(&mut positions, p0);
            push_2dpoint(&mut positions, p1);
            push_2dpoint(&mut positions, p2);
        }
        positions
    }
    ...
    pub fn non_indexed_normals_buffer(&self) -> Vec<f64> {
        ...
        for face_id in self.face_iter() {
            ...
            push_2dpoint(&mut normals, self.vertex_normal(v0));
            push_2dpoint(&mut normals, self.vertex_normal(v1));
            push_2dpoint(&mut normals, self.vertex_normal(v2));
        }
        normals
    }
}

fn push_2dpoint<P: TwoDimensionalPoint>(store: &mut Vec<P::SCALAR>, point: P) {
    for element in point.to_array() {
        store.push(element)
    }
}
