...
impl<P: TwoDimensionalPoint> Mesh<P> {
    ...
    pub fn smooth_vertices(&mut self, factor: f64) {
        ...
        for vertex_id in self.vertex_iter() {
            let mut avg_pos = P::new_point(P::SCALAR::zero(), P::SCALAR::zero());
            let mut i = 0;
    ...
    pub fn flip_edges(&mut self, flatness_threshold: f64) {
        let insert_or_remove =
            |mesh: &Mesh<P>, to_be_flipped: &mut HashSet<HalfEdgeID>, halfedge_id: HalfEdgeID| {
...
fn triangle_quality<P: TwoDimensionalPoint>(p0: &P, p1: &P, p2: &P) -> f64 {
