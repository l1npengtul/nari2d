...
#[derive(Clone, Debug)]
pub(crate) struct ConnectivityInfo<P: TwoDimensionalPoint> {
    vertices: RefCell<IDMap<VertexID, P>>,
    halfedges: RefCell<IDMap<HalfEdgeID, HalfEdge>>,
    faces: RefCell<IDMap<FaceID, Face>>,
}

impl<P: TwoDimensionalPoint> ConnectivityInfo<P> {
    pub fn new(no_vertices: usize, no_faces: usize) -> ConnectivityInfo<P> {
        ConnectivityInfo {
    ...
    pub fn new_vertex(&self, position: P) -> VertexID {
    ...
    pub fn position(&self, vertex_id: VertexID) -> P {
        RefCell::borrow(&self.vertices)
            .get(vertex_id)
            .unwrap()
            .position
    ...
    pub fn set_position(&self, vertex_id: VertexID, position: P) {
        RefCell::borrow_mut(&self.vertices)
            .get_mut(vertex_id)
            .unwrap()
            .position = position;
...
impl<P: TwoDimensionalPoint> std::fmt::Display for ConnectivityInfo<P> {
...
pub struct Vertex<P: TwoDimensionalPoint> {
