...
impl<P: TwoDimensionalPoint> Mesh<P> {
    ...
    pub fn move_vertex_to(&mut self, vertex_id: VertexID, value: P) {
    ...
    pub fn move_vertex_by(&mut self, vertex_id: VertexID, value: P) {
    ...
    /// ```
    ///
    pub fn non_uniform_scale(&mut self, scale_x: f64, scale_y: f64) {
        for vertex_id in self.vertex_iter() {
            let p = self.vertex_position(vertex_id);
            self.move_vertex_to(vertex_id, P::new_point(p.x() * scale_x, p.y() * scale_y));
        }
    }
    ...
    /// ```
    ///
    pub fn translate(&mut self, translation: P) {
        for vertex_id in self.vertex_iter() {
